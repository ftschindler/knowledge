...
try:
    log = check_output(cmd, text=True, stderr=DEVNULL).strip()
except CalledProcessError:
    # A repository without commits has no authors to check. `git log` fails
    # rather than returning nothing, so this is the bootstrap case, not an error.
    sys.exit(0)
seen_set = set()
seen = defaultdict(list)
for user in (f.split(fmtsep) for f in set(log.split("\n"))):
    if user[0] != user[1]:
        # has a mailmap entry
        continue
