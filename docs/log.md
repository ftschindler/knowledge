...
- take the pacing rule back over the thirteen densest pages, one commit each, measured by counting body sentences over forty words: ninety-nine across the bundle before, sixty-one after, and none left in the pages that were worked
- itemise rather than reword wherever the prose was carrying a list in disguise: the helper scripts of project-wiki, the kb skills, the method of two source reads, the ingest steps of the federation architecture
- leave the two pages a human has verified untouched, since rewriting the body under someone else's `verified:` would re-assert a check they never made
