...
Tools and projects worth their own page: what each one is, who makes it, and what it is for.
Mostly someone else's, occasionally mine.
...
Something I wrote belongs here too, tagged `self-authored` rather than given its own directory.
Who made a thing is a fact about it, not a kind of page, and a `projects/` folder would be the
domain axis
...
project rather than a product, and those names collide with each other and with things that are
not tools at all: "agent-wiki", "agent-knowledge" and "project-wiki" are three pages in this
directory and could as easily have been one. Who wrote a tool is also the thing most worth
knowing before depending on it, so it belongs where a reader lands rather than in a table
halfway down. Products with a vendor behind them keep their plain name, because `uv` is
Astral's and will not be anyone else's.
