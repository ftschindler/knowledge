...
- **R2** - no `raw/`-vs-rendered duplication for notes that are *already* markdown.
  ([awiki](../tools/agent_wiki_tacotakumi.md) copies an authored `.md` into `raw/` then renders a second page copy with
...
| 3 | Adopt a different engine: NimaChu/my-wiki or [stjbrown/agent-knowledge](../tools/agent_knowledge_stjbrown.md) | pass | pass | pass (skill optional) | pass/partial | md |
...
- **[stjbrown/agent-knowledge](../tools/agent_knowledge_stjbrown.md)** (MIT, active) - portable skills over plain markdown
